//! Stage-10 RAMBOOT: image-specific bootstrap plan using shared early-boot support.
use rk3588_boot_support::{SecondaryMailbox,MAILBOX,is_boot_cpu,relocation_words,runtime_address};
pub const COPY_END:u64=0x0300_C878;
pub const VBAR_EL3:u64=0x0300_B000;
pub const POST_RELOCATION_ENTRY:u64=0x0300_5A44;
...
#[cfg(test)]mod tests{use super::*;#[test]fn plan(){let p=bootstrap_plan(1,0x0300_1000);assert!(!p.boot_cpu);assert_eq!(p.post_relocation,POST_RELOCATION_ENTRY);assert_eq!(runtime_from_file(0x804),Some(0x0300_1004));}}
